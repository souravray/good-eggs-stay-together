...
func _physics_process(delta: float) -> void:
	if !_is_dead:
		...
		if is_jumping and !poewer_up and get_action_strength("power") > 0.0:
			if abs(_velocity.x) < 50.0:
				_velocity.y -= speed.y*0.6
			else:
				_velocity.y -= speed.y*1.1
			
			var directionChange = -1.0 if(_velocity.x>0 and direction.x<0) or (_velocity.x<0 and direction.x>0) else 1.0
			_velocity.x = _velocity.x * directionChange
